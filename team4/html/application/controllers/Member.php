<?
    class Member extends CI_Controller {       // Member클래스 선언
		...
		public function view()
		{
			...
			$data["text1"]=$text1;                      // text1 값 전달을 위한 처리
			$data["page"]=$page;
			$data["row"]=$this->member_m->getrow($ID);
		...
		{
			$uri_array=$this->uri->uri_to_assoc(3);
			$ID=array_key_exists("ID",$uri_array) ? $uri_array["ID"] : "" ;		// URI: /~team4/member/del/ID/1
			$text1=array_key_exists("text1",$uri_array) ? "/text1/" . urldecode($uri_array["text1"]) : "";
			$page = array_key_exists("page",$uri_array) ? "/page/" . urldecode($uri_array["page"]) : 0 ;
			
			$this->member_m->deleterow($ID);

			redirect("/~team4/member/lists/". $text1 . $page);             // 목록화면으로 돌아가기
		}
		...
		public function add()
		{
			...
			{
				$phone1 = $this->input->post("tel1",true);
				$phone2 = $this->input->post("tel2",true);
				$phone3 = $this->input->post("tel3",true);
				...
				redirect("/~team4/member/lists/" . $text1 . $page);    //   목록화면으로 이동.
			}
		}
		...
		public function edit()
		{
			...
			{  
				...
				$result = $this->member_m->updaterow($data,$ID);
				redirect("/~team4/member/lists/" . $text1 . $page);
			}
		}
	}
?>
