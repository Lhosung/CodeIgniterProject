<?
    class Booking extends CI_Controller {       // booking클래스 선언
        ...
        public function lists()
        {
			...
			$base_url = "/booking/lists/text1/$text1/text2/$text2/text3/$text3/page";    // $page_segment = 6;
			...
			$config["total_rows"] = $this->booking_m->rowcount($text1, $text2, $text3);  // 전체 레코드개수 구하기
			...
			$data["text2"]=$text2;
			$data["text3"]=$text3;
			$data["row_count"]= $this->booking_m->rowcount($text1, $text2, $text3);  // 전체 레코드개수 구하기
			$data["limit"] = $limit;
			$data["list"] = $this->booking_m->getlist($text1, $text2, $text3);   // 해당페이지 자료읽기
		...
		public function view()
		{
			...
			$data["text2"]=$text1;
			$data["page"]=$page;
			$data["row"]=$this->booking_m->getrow($ID);

			$this->load->view("admin_header");                    // 상단출력(메뉴)
            $this->load->view("booking_view",$data);           // booking_list에 자료전달
            $this->load->view("admin_footer"); 
		}
		public function add()
		{
			$data["list1"] = $this->booking_m->getlist_room();
			$data["list2"] = $this->booking_m->getlist_member();
			$uri_array=$this->uri->uri_to_assoc(3);
			...
			{ 
				...
				$this->load->view("booking_add", $data);    // 입력화면 표시
				$this->load->view("admin_footer");
			}
			...
			{				
				...
				redirect("/~team4/booking/lists" . $text1 . $text2 . $page);    //   목록화면으로 이동.
			}
		}
		...
		public function edit()
		{
			...
			{  	
				...
				$result = $this->booking_m->updaterow($data,$ID);
				redirect("/~team4/booking/lists" . $text1 . $page);
			}
		}
    }
?>
